...
paired_t_test_result <- data_tall %>%
  filter(Registered_Participation_Date < as.Date('2023-01-01')) %>%
...
#during and after their respective messaging period. Note that since the users are distributed in the date they recieved the first message and the time 
...
paired_t_test_result <- with(data, t.test(Rprts_During_Msging, Rprts_Before_Msging,  paired = TRUE))
print(paired_t_test_result)

paired_t_test_result <- with(data, t.test(Rprts_During_Msging, Rprts_After_Msging,  paired = TRUE))
print(paired_t_test_result)


# i chose not to mention the final comparision between after and before in the paper
paired_t_test_result <- with(data, t.test(Rprts_After_Msging, Rprts_Before_Msging,  paired = TRUE)) 
print(paired_t_test_result)
